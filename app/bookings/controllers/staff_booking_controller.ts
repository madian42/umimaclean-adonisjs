...
import { inspectionSchemaWithPhoto, uploadPhotoSchema } from '#bookings/validators/photo_validator'
...
import Service from '#transactions/models/service'
...
export default class StaffBookingController {
  ...
  async shipMode({ inertia, params, auth, response, session }: HttpContext) {
    ...
    try {
      ...
      if (params.stage === 'check') {
        // Show inspection interface
        return response.redirect().toRoute('staff.bookings.inspection.show', { id: booking.id })
      }
   ...
   */
  async uploadShipPhoto({ response, request, session, params, auth }: HttpContext) {
    const user = auth.getUserOrFail()
  ...
  async inspection({ inertia, session, params, response }: HttpContext) {
    // Load booking with all related data
    const booking = await Booking.query()
      .where('number', params.id)
      .preload('address')
    ...
    return inertia.render('bookings/staff/inspection', {
      booking,
      services,
      additional,
    })
  ...
  /**
   * Complete inspection stage with shoe details and photo
   *
   * Business Logic:
   * - Verifies staff has claimed the inspection stage
   * - Saves shoe inspection details (brand, type, size, material, condition, services)
   * - Uploads inspection photo
   * - Creates CHECK action for audit trail
   * - Updates booking status to WAITING_PAYMENT
   * - Uses transaction to ensure data consistency
   */
  async completeInspection({ response, request, session, params, auth }: HttpContext) {
    const user = auth.getUserOrFail()

    const payload = await request.validateUsing(inspectionSchemaWithPhoto)
    ...
    const attemptAction = await BookingAction.query()
      .where('booking_id', booking.id)
      .andWhere('action', BookingActions.ATTEMPT_CHECK)
      .andWhere('admin_id', user.id)
      .orderBy('created_at', 'desc')
      .first()
    ...
    try {
      ...
      for (const shoe of payload.shoes) {
        const createdShoe = await booking.related('shoes').create(
          {
            brand: shoe.brand,
            type: shoe.type,
            size: shoe.size,
      ...
      await BookingAction.create(
        {
          bookingId: booking.id,
          bookingPhotoId: bookingPhoto.id,
          adminId: user.id,
          action: BookingActions.CHECK,
          note: null,
        },
      ...
      // Update booking status to waiting payment
      const existingStatus = await BookingStatus.query({ client: trx })
        .where('booking_id', booking.id)
        .andWhere('name', BookingStatuses.WAITING_PAYMENT)
        .first()

      if (!existingStatus) {
        await BookingStatus.create(
          { bookingId: booking.id, name: BookingStatuses.WAITING_PAYMENT },
          { client: trx }
        )
      }
   ...
   */
  async releaseShipMode({ session, response, params, request, auth }: HttpContext) {
    const user = auth.getUserOrFail()
    const payload = request.only(['note'])

    const booking = await Booking.findBy('number', params.id)
    if (!booking) {
      session.flash('general_errors', 'Booking tidak ditemukan.')
      return response.redirect().back()
    }
    ...
    try {
      ...
      await BookingAction.create(
        {
          bookingId: booking.id,
          adminId: user.id,
          action: releaseActionMap[params.stage],
      ...
      await BookingStatus.query({ client: trx })
        .where('booking_id', booking.id)
        .andWhere('name', releaseStatusMap[params.stage])
        .delete()

...
export default function StaffBookingTabs({
  ...
  async function inspection(id: string) {
    router.visit(
      { route: 'staff.bookings.ship', params: { id, stage: 'check' } },
      {
        method: 'get',
        onError: (errors) => {
          if (errors?.general_errors) {
    ...
    <div className="space-y-4">
      {isLoading ? (
        ...
        items.map((booking) => {
          ...
          const bookingStatus = booking.statuses?.[0]?.name
          return (
            <Card key={booking.id}>
              <CardContent>
                ...
                <div
                  className={`space-y-2 text-sm ${
                    (bookingStatus === BookingStatuses.PICKUP_SCHEDULED && !booking.adminId) ||
                    booking.adminId === currentUserId ||
                    bookingStatus === BookingStatuses.PICKUP_PROGRESS ||
                    bookingStatus === BookingStatuses.DELIVERY ||
                    bookingStatus === BookingStatuses.INSPECTION
                      ? 'mb-4'
                      : ''
                  }`}
                >
                ...
                <div className="flex gap-2">
                  ...
                  {bookingStatus === BookingStatuses.INSPECTION && !booking.adminId && (
                    <Button size="sm" className="flex-1" onClick={() => inspection(booking.number)}>
                      <Camera className="w-4 h-4 mr-1" />
                      Pengecekan
                    </Button>
                  )}

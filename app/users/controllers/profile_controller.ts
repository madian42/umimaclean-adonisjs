import { updateNameSchema, updatePasswordSchema } from '#users/validators/profile_validator'
...
export default class ProfileController {
  async index({ inertia, request }: HttpContext) {
    ...
    return inertia.render('users/profile', { accordionState })
  }
  ...
  async updateName({ request, auth, response }: HttpContext) {
    const user = auth.getUserOrFail()
    const payload = await request.validateUsing(updateNameSchema)
